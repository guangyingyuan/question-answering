...
public class AskElasticService {
        ...
        if (response != null) {
            if (response.hits != null) {
                if (response.hits.hits != null) {
                    ...
                    if (hits != null) {
                        if (hits.size() > 0) {
                            hits = this.removeDuplicates(hits);
    ...
    private List<Hit> removeDuplicates (List<Hit> hits){
        List<Hit> output = new ArrayList<Hit>();
        boolean redundant = false;
        for (int indexHits = 0; indexHits < hits.size(); indexHits++) {
           for (int indexInner = indexHits + 1; indexInner < hits.size(); indexInner++) {
               String textAfter = getFullText(hits.get(indexInner)._source.text);
               String titleAfter = hits.get(indexInner)._source.title;
               String text = getFullText(hits.get(indexHits)._source.text);
               String title = hits.get(indexHits)._source.title;
               if ((textAfter.equals(text)) && (titleAfter.equals(title))){
                 redundant = true;
               }
           }
           if (redundant == false) {
             output.add(hits.get(indexHits));
           } else {
             redundant = false;
           }
        }
